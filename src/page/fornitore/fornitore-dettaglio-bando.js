  ...
  updatePartecipazione(){
    ...
    fetch("/api/updatePartecip", {
        ...
        if(data.affectedRows == 1){                               //controllo che la registrazione del bando sia avvenuta con successo se si
           ...
           self.onCloseModal('warningModal');              //tolgo il modal una volta che ho aggiornato la lista dei banditori
        }else{
  ...
  deletePartecipazione(){
    ...
    fetch("/api/deletePartecip", {
        ...
        if(data.affectedRows == 1){                               //controllo che la registrazione del bando sia avvenuta con successo se si
           ...
           self.onCloseModal('dangerModal');              //tolgo il modal una volta che ho aggiornato la lista dei banditori
           self.setState({PartecipCorr: [],
            ActionsInserimento: false,     //abilito i bottoni per l'inserimento
            ActionsPartecipa: true,      //abilito i bottoni per la modifica
            ActionsPartecipazione: true, //abilito i bottoni per la modifica
            PartecipCorrUpdate: []}) //aggiorno i valori dei campi del modal update        
        }else{
           eventBus.addNotification('error',"Abbiamo riscontrato problemi nell'eliminazione del bando, riprovare!");
        }
    }).catch(function(err) {
        console.log(err)
    });
  ...
  getPartecipazione(){
    ...
    fetch("/api/getProposta", {
        ...
        if(data.length != 0){                               //controllo che la registrazione del bando sia avvenuta con successo se si
           ...
           eventBus.addNotification('warning','Non hai partecipazioni a questa gara');
        }
    }).catch(function(err) {
        console.log(err)
    });
